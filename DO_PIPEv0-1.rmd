...
robustZmat <- function(phen, 
                       ...
                       pdfWid = 100,
                       pdfHei = 100,
                       path = "./diagnostic/phenotypes/Outlier-FullPheno.pdf",
                       zoom = FALSE,
                       valSize = 1){
  ...
  if(zoom == FALSE){
    ...
    pdf(path, width = pdfWid, height = pdfHei)
    par(mar=c(1,1,1,1))
...
covCheck <- function(cov, phen,
                     ...
                     margins = c(100,80),
                     rowFont = 15,
                     colFont = 10,                  path="./diagnostic/phenotypes/BatchEff_Pheno.pdf"){
  ...
  dumboDrop <- function(df){
    ...
    dumDown <- function(x, maxLev = 5){
      ...
      if(numLev == 1 | numLev > maxLev){
        ...
        dVar <- seq(0,numLev - 1)
        dumDF <- data.frame(var = unique(x), dVar = dVar)
        for(i in 1:numLev){
          x[x == dumDF[i,1]] <- dumDF[i,2]
        }
        return(as.numeric(x))
      }
    }
    covarDum <- lapply(cov, dumDown, maxLev = maxLev)
    covarDum <- as.data.frame(covarDum[!sapply(covarDum,
                                             is.null)])
    return(covarDum)
  }
...
########################
#Implement a try-catch that doesn't require database culling
enrichrMod <- function (genes, databases = NULL, quiet = F) 
{
    if(quiet == F){
...
In an empty project folder, place a copy of the DO_pipe markdown and run the Zero Arm chunks.

Add the following input files into the ./inputs folder:

  1. Final Report - must have the term "report" somewhere in the   file name. E.g. DOWL_finalreport.txt

  2. MUGA Folder - either MM or GM based on your study 

  3. Mouse_genes.sqlite - provided

  4. Master Phenotype - must have the term "pheno" somewhere in   the file name. E.g. DOWL_masterPheno.csv. 
...
```{r install/load necessary packages}
library(DBI)
library(httr)
library(qtl2)
library(dplyr)
...
if(!all(sapply(mPhen[,c(phenStart:ncol(mPhen))],is.numeric))){
  stop("You were about to put non-numeric data into the phenotype matrix. SHAME.\n")
}

#Write out the covar
write.csv(mPhen[,c(1:phenStart-1)], file = paste0("./inputs-processed/",prefix,"_covar.csv"), row.names = T)

#Write out the pheno
write.csv(mPhen[,c(phenStart:ncol(mPhen))], file = paste0("./inputs-processed/",prefix,"_pheno.csv"), row.names = TRUE)
```
...
robustZmat(phen,
           path = './diagnostic/phenotypes/Outlier-ZoomPheno.pdf',
           rowFont = 10,
           colFont = 20,
           zoom = T,
           valSize = 20,
...
covCheck(cov, phen,
         path = './diagnostic/phenotypes/BatchEff_Pheno.pdf',
         pdfWid = 100,
         pdfHei = 100,
         margins = c(100,80),
         rowFont = 15, 
...
permThresh <- readRDS(paste0("./scan-inputs/",prefix,"_permThresh.rds"))


#If your study has differing sexes, they must be submitted as a covariate AND an Xcovar argument
#Scan takes approx 4 sec/phenotype at 6 cores
system.time(
